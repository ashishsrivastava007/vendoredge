...
def normalize_evidence(
    ...
    if supplier_specific_evidence:
        for entry in supplier_specific_evidence:
            ...
            suppliers.append(SupplierEvidence(
                ...
                prior_otif_percent=_coerce_number(entry.get("prior_otif_percent"), f"supplier:{name}:prior_otif_percent", normalization_warnings, allow_percent_suffix=True),
                ...
                prior_defect_rate_percent=_coerce_number(entry.get("prior_defect_rate_percent"), f"supplier:{name}:prior_defect_rate_percent", normalization_warnings, allow_percent_suffix=True),
    ...
    if content_type == "price_increase":
        ...
        case_evidence = PriceIncreaseEvidence(
            ...
            specification_changed=llm_extracted_evidence.get("specification_changed") if isinstance(llm_extracted_evidence.get("specification_changed"), bool) else None,
            specification_change_description=_coerce_text(llm_extracted_evidence.get("specification_change_description"), "specification_change_description", normalization_warnings, numeric_scalar_ok=True),
